   ...
   // Add more methods to the model manager depending on what you wish to do
    ...
    public String[] getAllCountries()
    {
        ...
        for (int i = 0; i < allStudents.size(); i++)
        {
            ...
            if (!listOfCountries.contains(allStudents.get(i).getCountry()))
            {
                listOfCountries.add(allStudents.get(i).getCountry());
            }
        }

        String[] countryList = new String[listOfCountries.size()];
        return listOfCountries.toArray(countryList);
    }
}
